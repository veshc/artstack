...
name: regression
version: 0.2.0
description: Blast-radius analysis and regression suite selection for a change - what the change can break including across team boundaries, which suites to run with exact commands, what was excluded on purpose, where changed behaviour has no honest covering test, plus tests for the worst gaps and a residual-risk verdict. Use before merging to a release branch, before hardening, or when asked what a change might break. (artstack)
...
1. From the team file, note the test strategy, suite layout, and tagging. From the train file, note the dependency and NFR sections.
...
- Do not ignore consumers outside this repo; the dependency table in the train file exists for exactly this.
- Do not merge, push to a protected branch, or modify CI configuration. Committing the new regression tests to a feature branch is in scope; landing them is not.
